import java.sql.*;
public class test113130MOImportantBug {
    public static void main(String[] args) throws SQLException {
        String url1 = "jdbc:mysql://localhost:3306/test?user=root&password=1234&allowMultiQueries=false&rewriteBatchedStatements=true";
        String url2 = "jdbc:oceanbase://49.52.27.61:2881/test?user=root@test&password=1234&allowMultiQueries=false&rewriteBatchedStatements=false";
        Connection con = DriverManager.getConnection(url2);
        Statement stmt = con.createStatement();
        stmt.execute("DROP TABLE IF EXISTS t;");
        //con.createStatement().execute("CREATE TABLE t(id INT PRIMARY KEY,value BOOL);",1);
        stmt.execute(
                "CREATE TABLE t(" +
                        "  id INT PRIMARY KEY," +
                        "  value1 BIT," +
